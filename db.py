...
cursor.execute("CREATE TABLE IF NOT EXISTS users (uid INT, fio TEXT, selected_ticket INTEGER, current_question INTEGER, questions_passed INTEGER, questions_message INTEGER, in_process INTEGER)")
db.commit()
...
def add(uid: int):
    cursor.execute("INSERT INTO users (uid, fio, current_question, questions_passed, questions_message, in_process) VALUES (?, ?, ?, ?, ?, ?)", (uid, 0, 0, 0, 0, 0))
    db.commit()
...
def get_selected_ticket(uid: int):
    cursor.execute("SELECT selected_ticket FROM users WHERE uid=(?)", (uid,))
    return cursor.fetchone()[0]

def set_selected_ticket(uid: int, ticket: int):
    cursor.execute("UPDATE users SET selected_ticket=(?) WHERE uid=(?)", (ticket, uid))
    db.commit()

def get_fio(uid: int):
    cursor.execute("SELECT fio FROM users WHERE uid=(?)", (uid,))
    return cursor.fetchone()[0]

def set_fio(uid: int, fio: int):
    cursor.execute("UPDATE users SET fio=(?) WHERE uid=(?)", (fio, uid))
    db.commit()